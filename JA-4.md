...
性别只有两种，而相对应的，`boolean` 不是 `true` 就是 `false`，非常适合用于这种情况。

然而，事情不总是一帆风顺的，如果我们这么写，有一部分人就要掉小珍珠了 —— 包括但不限于中性、武装直升机和沃尔玛购物袋等。为了满足他们的需要，我们得设计一种类型，它能存储**比两种更多的状况**。

> Kirara：使用 `int` 或者 `String` 不就好了吗？
...
有谁知道这个 `2` 是什么意思？这会引发相当的混乱。

类似的，如果我们用中文写：

```java
String gender = "沃尔玛购物袋";
```

那么来自其它国家的工程师就很难理解我们的意思。

这个时候，我们就可以使用 Java 中的**枚举（Enum）**。枚举是一种**特殊的类**，它不需要定义属性或者方法，**只需要定义它所有可能的取值**。
